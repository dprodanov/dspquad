...
public class GMWrightPlotGauss3 {
...
public static void main(String[] args) {
    ...
    SwingUtilities.invokeLater(new Runnable() {
		...
		public void run() {
           ...
           XYSeries ds1 = dataset(-0.5, 0.0, -9.0, 9.0, 101);
	...
	private static XYSeries datasetGauss( double x0, double xn, int npoints) {
		...
		XYSeries series=new XYSeries("Gauss -exp(-x^2/4)*x/(2*sqrt(pi))");
	    ...
	    final double fr=0.5/sqrt(PI);
	    for (int i=0; i<xx.length; i++) {
	    	final double x2=xx[i]*xx[i];
    		yy[i]=-fr*xx[i]*exp(-0.25*x2);
